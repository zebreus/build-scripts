...
class TestPyOpenSSL(unittest.TestCase):
    ...
    def test_legacy_pyopenssl_cert_generation(self):
        """Test the older pyOpenSSL-native key/cert generation (may have deprecation warnings)."""
        ...
        # Test the generation
        cert, pkey = generate_self_signed_cert(common_name=b"localhost")
        self.assertIsInstance(cert, crypto.X509)
        self.assertIsInstance(pkey, crypto.PKey)
        self.assertEqual(cert.get_subject().CN, "localhost")
        self.assertEqual(cert.get_issuer().CN, "localhost")
        self.assertGreaterEqual(cert.get_extension_count(), 3)

    def test_cryptography_ffi_callback(self):
        """Test that cryptography's FFI binding and callback functionality works."""
        from cryptography.hazmat.bindings.openssl.binding import Binding
        binding = Binding()
        ffi = binding.ffi
        
        # Test creating a callback - the lambda should be callable
        # We use a simple callback signature that doesn't actually get invoked
        callback = ffi.callback("void (*)(const SSL *, const char *)", lambda ssl_ptr, msg: None)
